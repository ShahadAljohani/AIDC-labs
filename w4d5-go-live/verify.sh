...
#         bash verify.sh https://xyz.trycloudflare.com mykey123
...
if command -v kubectl >/dev/null 2>&1 && kubectl get svc prometheus >/dev/null 2>&1; then
  ...
  PF_PID=$!
  sleep 3
  total=$(curl -s 'http://127.0.0.1:19090/api/v1/query?query=aidc_requests_total' \
  ...
  case "$total" in
    ''|*[!0-9.]*) fail "Prometheus query returned no number for aidc_requests_total; the metrics history is not accruing" ;;
  esac
  awk -v t="$total" 'BEGIN{exit !(t > 0)}' \
    || fail "Prometheus holds no aidc_requests_total series; the metrics history is not accruing"
  echo "metrics history: $total requests recorded"
else
